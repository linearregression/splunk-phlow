...
# Berlin - Germany
#
# Modified for Postgresql by
# John Weir
#  when I started working on this script, it did not look
#  like it would work for MySQL.  It appeared very broken.
# 
# My comments are fairly sparse, and most of what you see
# are the original notes.  I will clean this up in time, but
# for now just wanted to get this script posted on github.
#
# There are two sql scripts included with this perl script.
# Use them to prepare the database. 
#
# ** create a database named 'ntop'
# 
# ** log in as ntop and run:
# 	function_add_cmt.sql
# 	ntop_flows.sql
#
# I received this script long ago, somewhere around '03 I would
# guess when I was setting up ntop.  This script was included.
# It has since been removed from code, and I haven't seen very
# much mention of databases for ntop in several years.
#
#
# Prerequisites:
# - Perl
...
# prints A LOT OF debug messages (MySQL queries)
my $debug  = 0;
my $debug2 = 0;

# shall we dump the raw flows into the 'flows' table
# (don't forget to create it first!)? 1 = true, 0 = false
my $rawFlows = 1;

# shall we ignore traffic the host in our localnets
# have among each other? 1 = true, 0 = false
my $ignoreLocalTraffic = 1;

# array containing our local nets we want to get stored
# notice: only /24 nets (256 hosts) are supported.
...
my @ip = ( '172.18.54.0', '172.20.28.0', '10.178.23.0', '10.178.54.0' );

# set your database parameters according to your needs

# Database name
my $databaseName = "DBI:Pg:dbname=ntop";

# Database user name
my $databaseUser = "netflow";

# Database user password
my $databasePw = "xxxxxxxx";
...
my $dbh = DBI->connect( $databaseName, $databaseUser, $databasePw )
...
# are we running yet? if not check if we
# should create any tables / data sets
my $run = 0;
my $test1;
...
while (1) {
    ...
    # Flow header
    my @recvdFlow = unpack(
        "a24 a48 a48 a48 a48 a48 a48 a48 a48 a48 a48 a48"
          . " a48 a48 a48 a48 a48 a48 a48 a48 a48 a48 a48"
          . " a48 a48 a48 a48 a48 a48 a48 a48",
        $buf
    );

    my ( $hver, $hcount, $hsysuptime, $hunix_secs, $hunix_nsecs,
        $hflow_sequence )
      = unpack( "n n N N N N x4", shift(@recvdFlow) );

    # We handle NetFlow 5 only
    next if ( $hver != 5 );

    # This packet doesn't seem to be ok
    next if ( $hcount != ( ( length($buf) - 24 ) / 48 ) );

    my $basetime =
      $hunix_secs + ( $hunix_nsecs / 1000000000 ) - ( $hsysuptime / 1000 );
    ...
    # check if date (year,month,day) changed
    if ( $now_time != $first_time || !$run ) {

        # if date changed check if month changed
        if ( $now_month != $first_month || !$run ) {
        ...
        # create table entries for today
        initializetable(2);
        $first_time = $now_time;

        # from now on we only check for the
        # tables when the date changed
        $run = 1;
    }
    ...
    while ( defined($aFlow) ) {
        ...
        # This flow is too small
        if ( length($aFlow) < 48 ) {
            last;
        }
        else {
            my (
                $fsrc,  $fdst,  $fnext, $fin,   $fout,  $fpkts,
                $focts, $fstrt, $fend,  $fsrcp, $fdstp, $ftcp,
                $fprot, $ftos,  $fsas,  $fdas,  $fsmsk, $fdmsk
            ) = unpack( "N3 n2 N4 n2 x C3 n2 C2 x2", $aFlow );

#print "hssysuptime=$hsysuptime\nhunix_secs=$hunix_secs\nhunix_nsecs=$hunix_nsecs\n\n";
#print "fstrt=$fstrt\nfend=$fend\n";
#print "basetime=$basetime\n";
#$test1 = (timestr($basetime+($fstrt/1000)));
#print "startTime=$test1\n";
            if ($rawFlows) {
                my $sql =
"INSERT INTO flows (ipSrc, ipDst, pktSent, bytesSent, startTime, endTime, srcPort, "
                  . "dstPort, tcpFlags, proto, tos) VALUES ('"
                  . num2ip($fsrc) . "', '"
                  . num2ip($fdst) . "', "
                  . "'$fpkts', '$focts', '"
                  . timestr( $basetime + ( $fstrt / 1000 ) ) . "', '"
                  . timestr( $basetime + ( $fend / 1000 ) )
                  . "', '$fsrcp', '$fdstp', '$ftcp', '$fprot', '$ftos')";
                if ($debug) { print $sql. "\n"; }
                $dbh->do($sql);
            }

            # check the incoming flow if one of the ip
            # fields belongs to our local nets
            foreach (@nets) {
                my $temp = $_;

                # source ip is in localnets, update bytesSent field
                if ( ( $fsrc & 0xffffff00 ) == $temp ) {

                    # we don't count traffic the hosts in our
                    # network list have among each other
                    if ($ignoreLocalTraffic) {
                        ...
                        foreach (@nets) {
                            ...
                            if ( ( $fdst & 0xffffff00 ) == $temp2 ) {
                                ...
                                # destination ip is in our localnets list
                                $inLocal = 1;
                                if ($debug2) {
                                    print "Lokaler Traffic: Von: "
                                      . num2ip($fsrc)
                                      . " Nach: "
                                      . num2ip($fdst) . "\n";
                                }
                  ...
                  # do the query if destionation ip is not in our localnets list
                        if ( !$inLocal ) {
                            my $sql2 =
"UPDATE traffic_$now_month SET bytesSent=bytesSent+$focts WHERE ip='"
                              . num2ip($fsrc)
                              . "' AND date='$now_time';";
                            $dbh->do($sql2);
                            if ($debug) { print $sql2. "\n"; }
...
"UPDATE traffic_$now_month SET bytesSent=bytesSent+$focts WHERE ip='"
                ...
                # destination ip is in localnets, update bytesRcvd field
                if ( ( $fdst & 0xffffff00 ) == $temp ) {

                    # we don't count traffic the hosts in
                    # our network list have among each othe
                    if ($ignoreLocalTraffic) {
                        my $inLocal = 0;
                        foreach (@nets) {
                            my $temp2 = $_;
                            if ( ( $fsrc & 0xffffff00 ) == $temp2 ) {
                                ...
                                # source ip is in our localnets list
                                $inLocal = 1;
                                if ($debug2) {
                                    print "Lokaler Traffic: Von: "
                                      . num2ip($fsrc)
                        ...
                        # do the query if source ip is not in our localnets list
                        if ( !$inLocal ) {
                            my $sql3 =
"UPDATE traffic_$now_month SET bytesRcvd=bytesRcvd+$focts WHERE ip='"
                              . num2ip($fdst)
                              . "' AND date='$now_time';";
                            $dbh->do($sql3);
                            if ($debug) { print $sql3. "\n"; }
...
sub timestr {
    ...
    # localtime() returns 0=Jan, 1=Feb, etc
    $mon++;
...
sub initializetable {
    ...
    if ( $mode == 1 ) {
        ...
        # create monthly table. mysql will check for us
        # that the table doesn't exist yet. ugly.
        ...
        #);";
        # replacement postgresql function call
        my $query = "select cmt('traffic_$now_month');";

        # primary key date is an index over the
        # date and ip fields which should make
        # the update queries for each incoming
        # flow much faster because it massively
        # speeds up searching for the dataset
        # specified in the WHERE clause
        $dbh->do($query) or die "Failed to create monthly traffic table.";

        # create daily entries in monthly table
    }
    elsif ( $mode == 2 ) {
        my $query2 =
"SELECT count(DISTINCT ip) AS c FROM traffic_$now_month WHERE date='$now_time'";
        my $countHosts = $dbh->selectrow_array($query2);

        # only do if the hosts for $now_time
        # don't exist yet. the script might come
        # here when started for the second
        # on one day
        if ( $numHosts != $countHosts ) {

            # go through our network list and create
            # an empty data set for each host
            foreach (@nets) {
                my $temp = $_;
...
#    			my $query3="INSERT IGNORE INTO traffic_$now_month VALUES('".num2ip($i)."',0,0,$now_time);";
                    my $query3 =
                        "INSERT INTO traffic_$now_month VALUES('"
                      . num2ip($i)
                      . "',0,0,$now_time);";
                    $dbh->do($query3);
                }
